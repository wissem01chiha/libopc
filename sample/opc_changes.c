...
typedef void(paragraph_callback_t)(void *callback_ctx, int level,
                                   xmlChar *modeTxt, xmlChar *parTxt);
...
static void initMode(changemode_t *mode) { opc_bzero_mem(mode, sizeof(*mode)); }
...
static void flush(context_t *ctx, int level) {
  if (NULL != ctx->callback_fct)
    ctx->callback_fct(ctx->callback_ctx, level, ctx->modeTxt, ctx->parTxt);
  if (NULL != ctx->modeTxt)
    xmlFree(ctx->modeTxt);
  ctx->modeTxt = NULL;
  if (NULL != ctx->parTxt)
...
static void par(context_t *ctx, int level, changemode_t *parMode,
                changemode_t *cellMode, changemode_t *rowMode) {
...
static void dumpText(context_t *ctx, mceTextReader_t *reader, int level,
                     changemode_t *textMode, changemode_t *parMode,
                     changemode_t *cellMode, changemode_t *rowMode,
                     changemode_t *prop_mode);
static void dumpChildren(context_t *ctx, mceTextReader_t *reader, int level,
                         changemode_t *textMode, changemode_t *parMode,
                         changemode_t *cellMode, changemode_t *rowMode,
                         changemode_t *prop_mode) {
  mce_start_children(reader) {
    mce_match_element(reader, NULL, NULL) {
      dumpText(ctx, reader, level, textMode, parMode, cellMode, rowMode,
               prop_mode);
    }
...
static void dumpText(context_t *ctx, mceTextReader_t *reader, int level,
  ...
  mce_start_choice(reader) {
    ...
    mce_start_element(reader, _X(ns_w), _X("ins")) {
      ...
      mce_start_attributes(reader) {
        mce_start_attribute(reader, _X(ns_w), _X("author")) {
          ins_props.mode = xmlStrcat(ins_props.mode,
      ...
      dumpChildren(ctx, reader, level, &ins_props, parMode, cellMode, rowMode,
                   prop_mode);
    ...
    mce_start_element(reader, _X(ns_w), _X("del")) {
      ...
      mce_start_attributes(reader) {
        mce_start_attribute(reader, _X(ns_w), _X("author")) {
          del_props.mode = xmlStrcat(del_props.mode,
                                     xmlTextReaderConstValue(reader->reader));
        }
        mce_end_attribute(reader);
      }
      ...
      dumpChildren(ctx, reader, level, &del_props, parMode, cellMode, rowMode,
                   prop_mode);
    ...
    mce_start_element(reader, _X(ns_w), _X("p")) {
      ...
      mce_start_children(reader) {
        ...
        mce_match_element(reader, NULL, NULL) {
          dumpText(ctx, reader, level, textMode, &p_props, cellMode, rowMode,
                   NULL);
        };
      }
      mce_end_children(reader);
    ...
    mce_start_element(reader, _X(ns_w), _X("tr")) {
      ...
      mce_start_children(reader) {
        mce_match_element(reader, _X(ns_w), _X("trPr")) {
          dumpText(ctx, reader, level + 1, textMode, parMode, cellMode, rowMode,
                   &tr_props);
        };
        mce_match_element(reader, NULL, NULL) {
          dumpText(ctx, reader, level + 1, textMode, parMode, cellMode,
                   &tr_props, NULL);
        };
      }
      mce_end_children(reader);
    ...
    mce_start_element(reader, NULL, NULL) {
      mce_skip_attributes(reader);
      dumpChildren(ctx, reader, level, textMode, parMode, cellMode, rowMode,
                   prop_mode);
    }
    mce_end_element(reader);
    mce_start_text(reader) {}
...
void parseText(xmlChar *filename, paragraph_callback_t *callback_fct,
               void *callback_ctx) {
  opcContainer *c = opcContainerOpen(filename, OPC_OPEN_READ_ONLY, NULL, NULL);
  if (NULL != c) {
    opcRelation rel =
        opcRelationFind(c, OPC_PART_INVALID, NULL,
                        _X("http://schemas.openxmlformats.org/officeDocument/"
    ...
    if (OPC_RELATION_INVALID != rel) {
      ...
      if (OPC_PART_INVALID != main) {
        ...
        if (0 == xmlStrcmp(type, _X("application/"
                                    "vnd.openxmlformats-officedocument."
                                    "wordprocessingml.document.main+xml"))) {
          mceTextReader_t reader;
          if (OPC_ERROR_NONE ==
              opcXmlReaderOpen(c, &reader, main, NULL, 0, 0)) {
            context_t ctx;
...
static void paragraph_callback(void *callback_ctx, int level, xmlChar *modeTxt,
                               xmlChar *parTxt) {
  if (NULL != modeTxt) {
