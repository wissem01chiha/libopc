...
static void addBinding(SourceContext *sc, const xmlChar *p, const xmlChar *ns) {
  puint32_t i = 0;
  while (i < sc->binding_len && 0 != xmlStrcmp(sc->binding_array[i].p, p))
    i++;
  if (i == sc->binding_len) {
    sc->binding_array = (SourceQName *)xmlRealloc(
        sc->binding_array, (sc->binding_len + 1) * sizeof(SourceQName));
    i = sc->binding_len++;
  ...
  if (NULL != sc->binding_array && i < sc->binding_len) {
    if (NULL != sc->binding_array[i].ns)
      xmlFree(sc->binding_array[i].ns);
...
static puint32_t addDef(SourceContext *sc, xmlChar *def) {
  sc->def_array = (SchemaDefinition *)xmlRealloc(
      sc->def_array, (sc->def_len + 1) * sizeof(SchemaDefinition));
...
static puint32_t findDef(SourceContext *sc, xmlChar *def) {
  for (puint32_t i = 0; i < sc->def_len; i++) {
    if (NULL != sc->def_array[i].name &&
        0 == xmlStrcmp(sc->def_array[i].name, def)) {
      return i;
    }
  }
  return -1;
...
static SchemaDeclaration *addDecl(SchemaDeclaration **decl_array,
                                  puint32_t *decl_len, const xmlChar *ns,
                                  const xmlChar *ln) {
  *decl_array = (SchemaDeclaration *)xmlRealloc(
      *decl_array, (1 + *decl_len) * sizeof(SchemaDeclaration));
  if (NULL != *decl_array) {
    memset((*decl_array) + (*decl_len), 0, sizeof((*decl_array)[(*decl_len)]));
    (*decl_array)[(*decl_len)].def_id = -1;
    if (NULL != ns)
      (*decl_array)[(*decl_len)].ns = xmlStrdup(ns);
    if (NULL != ln)
      (*decl_array)[(*decl_len)].ln = xmlStrdup(ln);
...
static void freeDecls(SchemaDeclaration **decl_array, puint32_t *decl_len) {
  if (NULL != *decl_array) {
    for (puint32_t j = 0; j < *decl_len; j++) {
      if (NULL != (*decl_array)[j].ln)
        xmlFree((*decl_array)[j].ln);
      if (NULL != (*decl_array)[j].ns)
        xmlFree((*decl_array)[j].ns);
    }
    xmlFree(*decl_array);
  }
}

static void readParam(SourceContext *sc, xmlChar *line, int *ofs, char *param,
                      int param_max) {
  int j = 0;
...
LineTokenId_t readLineToken(SourceContext *sc, LineTokenInstance *ti) {
  ...
  while (!sc->error_flag) {
    ...
    if (NULL != line) {
      int ofs = -1;
      int mce_def_ofs = -1;
      if ('#' == line[0] && 'd' == line[1] && 'e' == line[2] &&
          'f' == line[3] && 'i' == line[4] && 'n' == line[5] &&
          'e' == line[6]) {
        // filter out #define s
        ...
        if ('m' == line[0] && 'c' == line[1] && 'e' == line[2] &&
            '_' == line[3]) {
          ofs = 0;
        } else if ('}' == line[0] && 'm' == line[1] && 'c' == line[2] &&
                   'e' == line[3] && '_' == line[4]) {
          ofs = 1;
        }
        if (ofs >= 0) {
          int t = 0;
          while (LINE_TOKEN_ID_INVALID != s_token[t].id &&
                 0 != xmlStrncmp(_X(s_token[t].name), line + ofs,
                                 xmlStrlen(_X(s_token[t].name))))
            t++;
          if (LINE_TOKEN_ID_INVALID != s_token[t].id) {
            ti->id = s_token[t].id;
            PASSERT(0 == xmlStrncmp(_X(s_token[ti->id].name), line + ofs,
                                    xmlStrlen(_X(s_token[ti->id].name))));
            ofs += xmlStrlen(_X(s_token[t].name));
            readParam(NULL, line, &ofs, ti->param[0], sizeof(ti->param[0]));
            if (line[ofs] == ',') {
              ofs++;
              readParam(sc, line, &ofs, ti->param[1], sizeof(ti->param[1]));
            }
            if (line[ofs] == ',') {
              ofs++;
              readParam(sc, line, &ofs, ti->param[2], sizeof(ti->param[2]));
            }
          } else {
...
static void errorToken(SourceContext *sc, LineTokenInstance *ti) {
  printf("ERROR %s(%s, %s, %s)\n", s_token[ti->id].name, ti->param[0],
         ti->param[1], ti->param[2]);
  ti->id = LINE_TOKEN_ID_EOF;
  sc->error_flag = PTRUE;
}

static pbool_t parseDeclarations(SourceContext *sc, LineTokenInstance *ti,
                                 puint32_t def_id);
static pbool_t parseStartDeclaration(SourceContext *sc, LineTokenInstance *ti,
                                     SchemaDeclaration **decl_array,
                                     puint32_t *decl_len) {
  if (LINE_TOKEN_ID_START_ELEMENT == ti->id ||
      LINE_TOKEN_ID_START_ATTRIBUTE == ti->id ||
      LINE_TOKEN_ID_START_TEXT == ti->id) {
    LineTokenId_t const end_id =
        (LINE_TOKEN_ID_START_ELEMENT == ti->id     ? LINE_TOKEN_ID_END_ELEMENT
         : LINE_TOKEN_ID_START_ATTRIBUTE == ti->id ? LINE_TOKEN_ID_END_ATTRIBUTE
         : LINE_TOKEN_ID_START_TEXT == ti->id      ? LINE_TOKEN_ID_END_TEXT
                                                   : ti->id);

    SchemaDeclaration *decl =
        addDecl(decl_array, decl_len, _X(ti->param[1]), _X(ti->param[2]));
...
static pbool_t parseDeclarations(SourceContext *sc, LineTokenInstance *ti,
  ...
  if (LINE_TOKEN_ID_START_CHILDREN == ti->id) {
    readLineToken(sc, ti); // consume start token
    while (parseStartDeclaration(sc, ti, &sc->def_array[def_id].decl_array,
                                 &sc->def_array[def_id].decl_len)) {
    }
    ...
    readLineToken(sc, ti); // consume start token
    while (parseStartDeclaration(sc, ti, &sc->def_array[def_id].attr_array,
                                 &sc->def_array[def_id].attr_len)) {
    }
...
static pbool_t parseStartDocument(SourceContext *sc, LineTokenInstance *ti) {
  if (LINE_TOKEN_ID_START_DOCUMENT == ti->id) {
    ...
    if (parseStartDeclaration(sc, ti, &sc->decl_array, &sc->decl_len)) {
      return (LINE_TOKEN_ID_END_DOCUMENT == ti->id
                  ? LINE_TOKEN_ID_INVALID != readLineToken(sc, ti)
                  : PFALSE); // consume end_document
    }
  }
  return PFALSE;
}
...
static void dumpDecl(SourceContext *sc, FILE *out, int indent,
                     SchemaDeclaration *decl);
static void dumpDef(SourceContext *sc, FILE *out, int indent,
                    puint32_t def_id) {
  printf("%*sattrs:\n", indent, "");
...
static void dumpDecl(SourceContext *sc, FILE *out, int indent,
                     SchemaDeclaration *decl) {
  pbool_t const has_children =
      (decl->def_id >= 0 && decl->def_id < sc->def_len &&
       NULL == sc->def_array[decl->def_id].name &&
       (sc->def_array[decl->def_id].attr_len > 0 ||
        sc->def_array[decl->def_id].decl_len > 0));
  fprintf(out, "%*s\"%s\" \"%s\" -> \"%s\" %s\n", indent, "", decl->ns,
          decl->ln, sc->def_array[decl->def_id].name,
          (has_children ? " {" : ""));
  if (has_children) {
...
int main(int argc, const char *argv[]) {
  ...
  if (NULL != sc.binding_array) {
    for (puint32_t i = 0; i < sc.binding_len; i++) {
      if (NULL != sc.binding_array[i].p)
        xmlFree(sc.binding_array[i].p);
      if (NULL != sc.binding_array[i].ns)
        xmlFree(sc.binding_array[i].ns);
    }
